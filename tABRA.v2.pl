...
unless(-f "$sample.trinity.Trinity.fasta" ){
	system("Trinity --seqType fq --max_memory 30G --bypass_java_version_check --left $left_reads --right $right_reads --CPU $triCpus --bflyHeapSpaceMax 20G --bflyCPU $bflyCpus --KMER_SIZE 31 --SS_lib_type RF --min_kmer_cov 10 --min_glue 10  --output $sample.trinity --full_cleanup");
}
...
sub removeRedundantUpdateTpm{
        ...
        foreach my $seq(@sorted){
                ...
                foreach my$uSeq(@uniqSeq){
                        ...
                        $uSeqBL=~ s/\*//g;
                        if($uSeqBL eq $seqBL){
                                if($isoPcL{$uTid}->[1] >= $isoPcL{$pTid}->[1]){
